...
LATEST_TAG=$(git ls-remote --tags --refs https://github.com/orange-hive/docker-proxy.git | awk -F/ ' { print $3 }' | sed "s/release-//" | sort -t. -s -k 1,1n -k 2,2n -k 3,3n | tail -n 1)
...
if [ "$CURRENT_VERSION" == "$LATEST_TAG" ]; then
    ...
    case $response in
        [yY][eE][sS]|[yY])
            ...
            git clone --branch release-$LATEST_TAG https://github.com/orange-hive/docker-proxy.git .
            rm -Rf .git
